...
This file provides guidance and memory for GitHub Copilot CLI.

<!-- BEGIN: BMAD-AGENTS-COPILOT -->
# BMAD-METHOD Agents and Tasks (GitHub Copilot CLI)

GitHub Copilot CLI reads AGENTS.md during initialization and uses it as part of its system prompt for the session. This section is auto-generated by BMAD-METHOD for GitHub Copilot CLI.

## How To Use With GitHub Copilot CLI

- Run `gh copilot` in this project. GitHub Copilot CLI will read `AGENTS.md` and apply the agent definitions.
...
- Refresh this section after BMAD updates: `npx bmad-method install -f -i opencode` (or manually update references).
...
### Package Management
- **ALWAYS use `pnpm`** for all package operations (install, add, remove, run)
- Never use npm or yarn commands

### Code Style & Architecture
- TypeScript strict mode enabled (no `any` types)
- Use functional components (React Server Components by default)
...
- Reference Epic/Story in commit body when applicable
- Can use `./smart-commit.sh` for automated commits
- Include Copilot CLI metadata in commit footers

### Testing Priorities
1. E2E tests with Playwright (Happy Path coverage)
2. Unit tests only for complex business logic
3. Test files colocated: `*.test.tsx` alongside components

### Documentation Requirements
- Update `DEVELOPMENT_LOG.md` after each significant session
- Update `COPILOT_FEATURES.md` when using new CLI capabilities
- Keep architecture.md synchronized with code changes
- Document all Epic/Story implementations
